...
const CustomTextFieldWithDropdown: React.FC<
  CustomTextFieldWithDropdownProps
> = ({ filter, handleValueChange }) => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      (prevComparison === "IN" || prevComparison === "NOT IN") &&
  ...
  useEffect(() => {
    if (filter.column) {
      const fetchOptions = async () => {
        try {
          const mediaToFilter =
            mediaIndices.length > 0 ? mediaIndices.slice(-1)[0] : [];
          const response = await axios.post<Option[]>(
            `${API_URL}/views/${uuid}/columns/${filter.column}/values`,
            {
              media_ids: mediaToFilter,
            },
          );
  ...
  return (
    <div className="relative inline-block w-full mb-2" ref={dropdownRef}>
      <input
        type="text"
        className="w-full bg-white border border-gray-300 text-gray-700 py-2 px-4 pr-8 rounded leading-tight focus:outline-none focus:ring-blue-500 focus:border-blue-500"
        value={inputValue}
        ...
        <ul className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto">
          {options
            ...
            .map((option) => (
              ...
              >
                {option.label}
                <span className="bg-gray-100 rounded text-2xs text-gray-500 p-1 ml-2">
                  {option.count}
                </span>
              </li>
            ))}
      ...
      {filter.values.length > 0 && (
        <div className="mt-2 flex flex-wrap">
          {filter.values.map((value) => (
            <span
              key={value}
              className="bg-blue-900 text-white py-1 px-2 rounded-full mr-2 mb-2 flex items-center"
            >
              {value}
              <button
